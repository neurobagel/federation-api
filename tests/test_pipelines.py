...
@pytest.mark.parametrize("invalid_response", [{"nb:UnexpectedKey": []}, []])
def test_invalid_response_format_handled_gracefully(
    test_app, monkeypatch, set_valid_test_federation_nodes, invalid_response
):
    """
    Test that when one of the nodes returns a response without the expected key or shape,
    ...
    assert (
        "Unexpected response format" in response_object["errors"][0]["error"]
    )
